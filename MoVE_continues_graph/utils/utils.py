...
class NeighborSampler:
    ...
    def find_neighbors_before(self, node_id: int, interact_time: float, return_sampled_probabilities: bool = False):
        ...
        try:
            ...
            print("报错信息:", e)
            print("self.nodes_neighbor_times 长度:", len(self.nodes_neighbor_times))
            print("当前 node_id:", node_id)
            raise
            ...
            for hop in range(1, num_hops):
                ...
                if len(nodes_neighbor_ids) + len(nodes_neighbor_ids_list[0][i]) >= num_neighbors and hop >= 2:
                    ...
                    if len(last_hop_node_neighbor_ids) != len(nodes_neighbor_ids):
                        nodes_neighbor_ids, nodes_edge_ids, nodes_neighbor_times = self.get_community_historical_neighbors(
                            node_ids=np.array(
                                nodes_neighbor_ids_list[-1][i] if hop == 1 else nodes_neighbor_ids_list[-1][i][
                                                                                -len(last_hop_node_neighbor_ids):]),
                            node_interact_times=np.array(
                                nodes_neighbor_times_list[-1][i] if hop == 1 else nodes_neighbor_times_list[-1][i][
                                                                                  -len(last_hop_node_neighbor_ids):]),
                            num_neighbors=num_neighbors)
                    for i in range(len(last_hop_node_neighbor_ids)):
...
class NegativeEdgeSampler(object):
    ...
    def sample(self, size: int, batch_src_node_ids: np.ndarray = None, batch_dst_node_ids: np.ndarray = None,
        ...
        """
        if self.negative_sample_strategy == 'random':
            # negative_src_node_ids, negative_dst_node_ids = self.random_sample_fixed(batch_src_node_ids=batch_src_node_ids,
            #                                                                         batch_dst_node_ids=batch_dst_node_ids, size=size)
    ...
    def random_sample_fixed(self, batch_src_node_ids: np.ndarray, batch_dst_node_ids: np.ndarray, size: int) -> Tuple[
        np.ndarray, np.ndarray]:
        """
        Randomly samples negative edges and ensures they are not 'false negatives'
        by checking against the provided true edges in the batch.

        :param batch_src_node_ids: np.ndarray, source node IDs of the true edges in the current batch.
        :param batch_dst_node_ids: np.ndarray, destination node IDs of the true edges in the current batch.
        :param size: int, the number of negative edges to sample.
        :return: A tuple of np.ndarrays for the source and destination nodes of the sampled negative edges.
        """
        # Step 1: Create a fast-lookup set of all true edges in the batch.
        # A set provides O(1) average time complexity for checking existence.
        true_edges: Set[Tuple[int, int]] = set(zip(batch_src_node_ids, batch_dst_node_ids))

        neg_src_nodes = []
        neg_dst_nodes = []

        # Use the appropriate random state for reproducibility
        rand_state = self.random_state if self.seed is not None else np.random

        # Step 2: Loop until we have collected the desired number of valid negative samples.
        while len(neg_src_nodes) < size:
            # Step 3: Sample a candidate negative edge from the global pool of unique nodes.
            candidate_src_idx = rand_state.randint(0, len(self.unique_src_node_ids))
            candidate_dst_idx = rand_state.randint(0, len(self.unique_dst_node_ids))

            candidate_src = self.unique_src_node_ids[candidate_src_idx]
            candidate_dst = self.unique_dst_node_ids[candidate_dst_idx]

            # Step 4: Check if the candidate edge is a true edge. If not, it's a valid negative sample.
            if (candidate_src, candidate_dst) not in true_edges:
                neg_src_nodes.append(candidate_src)
                neg_dst_nodes.append(candidate_dst)

        return np.array(neg_src_nodes), np.array(neg_dst_nodes)
    ...
    def streaming_topk(self, node_embeddings, community_score, all_community_member, community_embeddings, source_nodes, timestamps, edge_idxs):
        ...
        for index0, alpha in enumerate(self.alpha_list):
            ...
            for i in range(n_edges):
                ...
                if source != target:
                    PPR_list[source] = new_s1_PPR  # 对应πi,t-
                    PPR_list[target] = new_s2_PPR
                    norm_list[source] = norm_list[source] * beta + beta  # 对应mit-
        ...
        # for i in range(center_embeddings.shape[0]):
        #
        #     community_member.append(batch_node_list_0[i][topk_indices[i]].tolist())
        #     weight = all_indices[i][0][topk_indices[i]]
        #     node_idx = batch_node_list_0[i][topk_indices[i]].tolist()
        #     weighted_emb = torch.matmul(weight, node_embeddings[node_idx])
        #     community_embeddings[source_nodes[i]] = weighted_emb.sum(dim=0)

        # 获取 topk_indices 对应的节点和权重
        ...
        # 将结果保存到 community_embeddings
        community_embeddings[source_nodes] = weighted_embeddings

        # 找到当前batch的社区成员和社区中心embedding计算社区分数
        ...
        final_community_center = torch.topk(score.t(), 150)[1].squeeze(dim=0)
        ...
        # 将每个社区中心及其社区成员的embeddings统一
        community_embeddings[final_community_member] = community_embeddings[final_community_center].unsqueeze(
            dim=1).expand(-1, final_community_member.shape[1], -1)
