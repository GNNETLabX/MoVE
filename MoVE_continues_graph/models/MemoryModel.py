...
class MemoryModel(torch.nn.Module):
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray,
        ...
        updated_node_memories, updated_node_last_updated_times = self.get_updated_memories(node_ids=np.array(range(self.num_nodes),dtype=np.int64),
...
class GraphAttentionEmbedding(nn.Module):
    ...
    def compute_node_temporal_embeddings(self, node_memories: torch.Tensor, node_ids: np.ndarray, node_interact_times: np.ndarray,
        ...
        # shape (batch_size, node_feat_dim) add memory and node raw features to get node features note that when
        # using getting values of the ids from Tensor, convert the ndarray to tensor to avoid wrong retrieval
        ...
        if current_layer_num == 0:
            ...
            # Tensor, output shape (batch_size, node_feat_dim) follow the TGN paper, use merge layer to combine 1)
            # the attention results, and 2) node raw feature + node memory
