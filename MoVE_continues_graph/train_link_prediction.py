...
if __name__ == "__main__":
    ...
    for run in range(args.num_runs):
        ...
        elif args.model_name == 'MoVE':
            dynamic_backbone = MoVE(node_raw_features=node_raw_features, edge_raw_features=edge_raw_features,
                                    neighbor_sampler=train_neighbor_sampler,
                                    time_feat_dim=args.time_feat_dim, channel_embedding_dim=args.channel_embedding_dim,
                                    patch_size=args.patch_size,
                                    num_layers=args.num_layers, num_heads=args.num_heads, dropout=args.dropout, k=args.k,
        ...
        optimizer_env = torch.optim.Adam(
            [p for n, p in model.named_parameters() if "environment_estimator" in n],
            lr=0.0001, weight_decay=args.weight_decay
        )
        optimizer = torch.optim.Adam(
            [p for n, p in model.named_parameters() if "environment_estimator" not in n],
        ...
        for epoch in range(args.num_epochs):
            ...
            train_idx_data_loader_tqdm = tqdm(train_idx_data_loader, ncols=170)
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                ...
                _, batch_neg_dst_node_ids = train_neg_edge_sampler.sample(size=len(batch_src_node_ids), batch_src_node_ids=batch_src_node_ids, batch_dst_node_ids=batch_dst_node_ids)
                batch_src_node_ids = batch_src_node_ids.astype(np.int64)
                batch_dst_node_ids = batch_dst_node_ids.astype(np.int64)
                ...
                if args.model_name in ['TGAT', 'CAWN', 'TCL']:
                    ...
                    batch_neg_src_node_embeddings, batch_neg_dst_node_embeddings = model[
                        0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_neg_src_node_ids,
                                                                    ...
                                                                    num_neighbors=args.num_neighbors,
                                                                    time_gap=args.time_gap)
                elif args.model_name in ['MoVE']:

                    # get temporal embedding of negative source and negative destination nodes
                    # two Tensors, with shape (batch_size, node_feat_dim)
                    batch_neg_src_node_embeddings, batch_neg_dst_node_embeddings, _, _ = model[
                        0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_neg_src_node_ids,
                                                                    dst_node_ids=batch_neg_dst_node_ids,
                                                                    node_interact_times=batch_node_interact_times)
                    # get temporal embedding of source and destination nodes
                    # two Tensors, with shape (batch_size, node_feat_dim)
                    batch_src_node_embeddings, batch_dst_node_embeddings, align_loss, env_loss = model[
                        0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_src_node_ids,
                                                                    dst_node_ids=batch_dst_node_ids,
                                                                    node_interact_times=batch_node_interact_times)
                ...
                recon_loss = loss_func(input=predicts, target=labels)
                ...
                KL_loss = (1 / train_data_indices.shape[0]) * dynamic_backbone.kl_loss()
                loss = recon_loss + args.beta * KL_loss + args.alpha * align_loss
                ...
                # train_idx_data_loader_tqdm.set_description(f'Epoch: {epoch + 1}, train for the {batch_idx + 1}-th batch, train loss: {loss.item()},GPU Memory: {gpu_memory:.2f} MB')
                train_idx_data_loader_tqdm.set_description(
                    f'Epoch: {epoch + 1}, train for the {batch_idx + 1}-th batch, recon_loss: {recon_loss.item():.4f}, KL_loss: {args.beta * KL_loss.item():.4f}, align_loss: {args.alpha * align_loss.item():.4f}, env_loss: {env_loss.item():.4f}, total_loss: {loss.item():.4f}')
